...
/**
 * Shader-generation options (the `SHADER_FLAGS` of src/gpu/gen.ts), exposed in the preview toolbar.
 * Adding an option is two edits: a row in `ROWS` below and the flag itself in `ShaderFlags` — the
...
export function GenOptions({ flags, onChange, census }: { flags: GenFlags; onChange: (p: Partial<GenFlags>) => void; census: Census }) {
  ...
  return (
    <div className="relative" ref={box}>
      <button onClick={() => setOpen((v) => !v)} title="Shader generation options (branches, culling, loops)"
        className={cn("rounded-md border border-line px-2 py-0.5 font-mono hover:bg-surface-2", open && "bg-surface-3 text-fg")}>
        gen: {flags.branchless ? "branchless" : "branched"}
      </button>
      {open && (
        <div className="absolute right-0 z-30 mt-1 w-[19rem] rounded-lg border border-line bg-surface p-2 text-[11px] text-muted shadow-2xl shadow-black/60">
          <div className="px-1 pb-1.5 pt-0.5 font-semibold uppercase tracking-[0.12em] text-fg/70">shader generation</div>
          <div className="flex overflow-hidden rounded-md border border-line">
            {(["branched", "branchless"] as const).map((m, i) => (
              <button key={m} onClick={() => onChange({ branchless: i === 1 })}
                className={cn("flex-1 px-2 py-1 font-mono transition-colors", (flags.branchless ? i === 1 : i === 0) ? "bg-accent/80 text-white" : "hover:bg-surface-2")}>
                {m}
              </button>
            ))}
          </div>
          <div className="px-1 pt-1.5 text-[10px] leading-snug">
            {flags.branchless
              ? "no if / break / short-circuit at all: both arms of every branch are computed. Measured 3× slower in 2D and 10× in the 3D view (branchbench)."
              : "cull and early-out branches are kept — the fast build."}
          </div>
          ...
          <div className="mt-2 border-t border-line px-1 pt-1.5 font-mono text-[10px] text-fg/60">
