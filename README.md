...
panels — every seam between two regions is draggable (canvas/panels, editor/preview, panel/panel, and the reference drawer; double-click hands it back to the layout) — and any of the three folds to a thin labelled bar; the **`WGSL` / `JS` view** shows the *whole* shader the backend compiles (uniforms, entry point
