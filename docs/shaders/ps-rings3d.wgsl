...
fn stepf(q: vec3f) -> f32 {
  let p0 = q;
  let t0: f32 = (length(vec3f(p0.x, p0.y, 0.0)) - P[0]);
  let t1: f32 = (length(vec3f(t0, p0.z, 0.0)) - P[1]);
  let t2: vec4f = vec4f(P[2], P[3], P[4], P[5]);
  let t3: vec3f = (p0 - vec3f(P[6], P[7], P[8]));
  let t4: f32 = (((P[9] * t3.x) + (P[10] * t3.y)) + (P[11] * t3.z));
  let t5: f32 = (((P[12] * t3.x) + (P[13] * t3.y)) + (P[14] * t3.z));
  let t6: f32 = (((P[15] * t3.x) + (P[16] * t3.y)) + (P[17] * t3.z));
  let t7: f32 = (length(vec3f(vec3f(t4, t5, t6).x, vec3f(t4, t5, t6).y, 0.0)) - P[18]);
  let t8: f32 = (length(vec3f(t7, vec3f(t4, t5, t6).z, 0.0)) - P[19]);
  let t9: vec4f = vec4f(P[20], P[21], P[22], P[23]);
  let t10: f32 = min(t1, t8);
  let t11: vec4f = select(t2, t9, ((t8 <= 0.0) | (t8 <= t1)));
  let t12: vec3f = (p0 - vec3f(P[24], P[25], P[26]));
  let t13: f32 = (((P[27] * t12.x) + (P[28] * t12.y)) + (P[29] * t12.z));
  let t14: f32 = (((P[30] * t12.x) + (P[31] * t12.y)) + (P[32] * t12.z));
  let t15: f32 = (((P[33] * t12.x) + (P[34] * t12.y)) + (P[35] * t12.z));
  let t16: f32 = (length(vec3f(vec3f(t13, t14, t15).x, vec3f(t13, t14, t15).y, 0.0)) - P[36]);
  let t17: f32 = (length(vec3f(t16, vec3f(t13, t14, t15).z, 0.0)) - P[37]);
  let t18: vec4f = vec4f(P[38], P[39], P[40], P[41]);
  let t19: f32 = min(t10, t17);
  let t20: vec4f = select(t11, t18, ((t17 <= 0.0) | (t17 <= t10)));
  return t19;
}
