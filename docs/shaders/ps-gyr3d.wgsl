...
fn stepf(q: vec3f) -> f32 {
  let p0 = q;
  let t0: vec3f = (p0 / vec3f(P[1], P[2], P[3]));
  let t1: f32 = (u.cam.z * P[4]);
  let t2: vec4f = vec4f(t0, u.time);
  let t3: f32 = (((cos(t2.x) * sin(t2.y)) + (cos(t2.y) * sin(t2.z))) + (cos(t2.z) * sin(t2.x)));
  let t4: vec4f = vec4f(0.86, 0.82, 0.55, 1.0);
  let t5: f32 = (t3 - P[5]);
  let t6: f32 = (t5 / P[6]);
  let t7: f32 = (t6 * P[4]);
  let t8: f32 = (length(p0) - P[7]);
  let t9: f32 = clamp((0.5 + ((0.5 * ((-t8) - (-t7))) / max(P[0], 0.0001))), 0.0, 1.0);
  let t10: f32 = (-(mix((-t8), (-t7), t9) - ((P[0] * t9) * (1.0 - t9))));
  let t11: vec4f = select(t4, vec4f(0.86, 0.82, 0.55, 1.0), (t8 > t7));
  let t12: vec4f = vec4f(P[8], P[9], P[10], P[11]);
  return t10;
}
